...
server.tool("scrape_links", "Scrape multiple URLs and return clean, structured content", scrapeLinksSchema, async ({ links }) => {
...
server.tool("x_search", "Search X/Twitter for people, scrape profiles, and return normalized data", xSearchSchema, async ({ query, scrape_profiles, max_profiles }) => {
    const controller = new AbortController();
...
server.tool("insta_search_scrape", "Search Instagram, get top profiles, and scrape with flexible DOM handling", instaSearchScrapeSchema, async ({ query, max_profiles }) => {
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), 15000);
    // Step 1: Search for profiles
    const params = new URLSearchParams({
...
server.tool("adult_search", "Search adult content safely with flexible DOM handling", adultSearchSchema, async ({ query, max_results, content_type }) => {
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), 15000);
    // Step 1: Search with adult content filters
    let searchQuery = query;
    if (content_type === "videos")
        searchQuery += " videos porn";
    else if (content_type === "images")
        searchQuery += " images porn";
    else if (content_type === "profiles")
        searchQuery += " profiles porn";
    else
        searchQuery += " porn";
    const params = new URLSearchParams({
        q: searchQuery,
    ...
    const contentResults = (data.results || [])
        ...
        const hasAdultKeywords = adultKeywords.some(keyword => url.includes(keyword) || title.includes(keyword) || content.includes(keyword));
    ...
    const scrapedContent = await Promise.all(contentResults.map(async (content) => {
        try {
...
async function scrapeAdultContent(url) {
    try {
        const controller = new AbortController();
        const timeout = setTimeout(() => controller.abort(), 10000);
        const response = await fetch(url, {
...
server.tool("youtube_transcript", "Get transcript from YouTube video using youtube-transcript package", youtubeTranscriptSchema, async ({ video_url, language }) => {
    try {
        ...
        const formattedTranscript = segments
            .map((segment) => `[${(segment.duration || segment.offset || 0).toFixed(2)}s] ${segment.text || segment.content}`)
            .join('\n');
        // Get total duration
        const totalDuration = segments.reduce((sum, segment) => sum + (segment.duration || segment.offset || 0), 0);
        const output = {
            video_url: video_url,
            video_id: videoId,
...
server.resource("websearch", "mcp://websearch", {
...
server.tool("get_links", "Search SearXNG and return up to 6 links", getLinksSchema, async ({ query, categories }) => {
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), 15000);
    const params = new URLSearchParams({
        q: query,
    ...
    const unresp = data.unresponsive_engines && data.unresponsive_engines.length

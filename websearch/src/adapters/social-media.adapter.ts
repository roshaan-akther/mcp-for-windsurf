  ...
  handler: async ({ query, scrape_profiles, max_profiles }) => {
    ...
    if (scrape_profiles) {
      scrapedProfiles = await Promise.all(
        profileResults.map(async (profile: any) => {
          try {
...
const instaSearchTool: Tool = {
  ...
  handler: async ({ query, max_profiles }) => {
    ...
    const output = {
      ...
      search_summary: {
        total_profiles_found: profileResults.length,
        successfully_scraped: scrapedProfiles.filter((p: any) => !p.error).length,
        failed_scrapes: scrapedProfiles.filter((p: any) => p.error).length,
      },
      profiles: scrapedProfiles.map((p: any) => p.scraped || {
...
export const socialMediaAdapter: Adapter = {
  name: "social-media",
